...
//mixed radix-97  radix-11 , radix-7 , radix-5  , radix-4  , radix-3  , radix-2 FFT is little quicker that previous version because it calculates FFT witch tables instead of complex number objects
...
void fun_fourier_transform_FFT_radix_97_stg_rest(std::complex<double> tab[],int nb1,int nb2,int nb3,int nb4,int nb5_stg_previous,double tmp99,double (*z)[2]);
...
void fun_fourier_transform_FFT_radix_97_stg_first(std::complex<double> tab[],int stg_first,double fi2,double (*z)[2]);
...
//radix 11 fundament
          ...
          for (int i2=0;i2<97;i2++)
        {
            ...
            {
                z_rx97[i2*j2][0]=cos(i2*j2*tmp97);
                z_rx97[i2*j2][1]=-sin(i2*j2*tmp97);
            }
        }
 ...
 void fun_fourier_transform_FFT_radix_97_stg_rest(std::complex<double> tab[],int nb1,int nb2,int nb3,int nb4,int nb5_stg_previous,double tmp99,double  (*z)[2])
    {
        ...
        for(int b=0;b<nb5_stg_previous;b=b+1)
        {
            ...
            for(int j=0;j<nb3;j=j+1)
            {
                ...
                {
                    w[j9][0]=(cos(j9*tmp100+tmp200));
                    w[j9][1]=(-sin(j9*tmp100+tmp200));
                }
                ...
                for(int i=0;i<nb4;i=i+1)
                {
                  ...
                  for(int j9=0;j9<97;j9++)
                  {
                        tmp1[j9][0]=w[j9][0]*tab[nb_tmp3+j9*nb3].real()-w[j9][1]*tab[nb_tmp3+j9*nb3].imag();
                        tmp1[j9][1]=w[j9][0]*tab[nb_tmp3+j9*nb3].imag()+w[j9][1]*tab[nb_tmp3+j9*nb3].real();
                  }
                  ...
                  for(int i9=0;i9<97;i9++)
                  {
                    for(int j9=0;j9<97;j9++)
                    {
                        tab[nb_tmp3+i9*nb3].real(tab[nb_tmp3+i9*nb3].real()+z[i9*j9][0]*tmp1[j9][0]-z[i9*j9][1]*tmp1[j9][1]);
                        tab[nb_tmp3+i9*nb3].imag(tab[nb_tmp3+i9*nb3].imag()+z[i9*j9][0]*tmp1[j9][1]+z[i9*j9][1]*tmp1[j9][0]);
                    }
                  }
                }
 ...
 void fun_fourier_transform_FFT_radix_97_stg_first(std::complex<double> tab[],int stg_first,double fi2,double (*z)[2])
  {
	...
	for(int j9=0;j9<97;j9++)
	{
        ...
        for(int i=0;i<stg_first;i=i+1)
        {
          for(int j9=0;j9<97;j9++)
          {
                tmp1[j9][0]=w[j9][0]*tab[i+j9*stg_first].real()-w[j9][1]*tab[i+j9*stg_first].imag();
                tmp1[j9][1]=w[j9][0]*tab[i+j9*stg_first].imag()+w[j9][1]*tab[i+j9*stg_first].real();
          }
          ...
          for(int i9=0;i9<97;i9++)
          {
            for(int j9=0;j9<97;j9++)
            {
                tab[i+i9*stg_first].real(tab[i+i9*stg_first].real()+z[i9*j9][0]*tmp1[j9][0]-z[i9*j9][1]*tmp1[j9][1]);
                tab[i+i9*stg_first].imag(tab[i+i9*stg_first].imag()+z[i9*j9][0]*tmp1[j9][1]+z[i9*j9][1]*tmp1[j9][0]);
            }
          }
        }
    }
